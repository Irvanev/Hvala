...
function getCountryKey(string) {
  if (
    string.includes("Serbia") ||
    string.includes("Сербия") ||
    string.includes("Србија")
  ) {
    return "serbia";
  } else if (
    string.includes("Croatia") ||
    string.includes("Хорватия") ||
    string.includes("Хрватска")
  ) {
    return "croatia";
  } else if (
    string.includes("Bosnia and Herzegovina") ||
    string.includes("Босния и Герцеговина") ||
    string.includes("Босна и Херцеговина")
  ) {
    return "bosnia_and_herzegovina";
  } else if (
    string.includes("Montenegro") ||
    string.includes("Черногория") ||
    string.includes("Црна Гора")
  ) {
    return "montenegro";
  } else if (
    string.includes("North Macedonia") ||
    string.includes("Мacedonia") ||
    string.includes("Северная Македония") ||
    string.includes("Македония") ||
    string.includes("Северна Македонија")
  ) {
...
function getRegionKey(string) {
  if (
    ...
    string.includes("Southern and Eastern Serbia") ||
    string.includes("Южно-Банатский")
  ) {
    return "southern_and_eastern_serbia";
  } else if (
    string.includes("Belgrade") ||
    string.includes("Белград") ||
    string.includes("Београд")
  ) {
    return "belgrade";
  } else if (
    string.includes("Bor") ||
    string.includes("Bor") ||
    string.includes("Борский") ||
    string.includes("Борски")
  ) {
    return "bor_district";
  } else if (
    string.includes("Braničevo District") ||
    string.includes("Braničevo") ||
    string.includes("Браничевский") ||
    string.includes("Браничевски округ")
  ) {
    return "branicevo_district";
  } else if (
    string.includes("Zlatibor District") ||
    string.includes("Zlatibor") ||
    string.includes("Златиборский") ||
    string.includes("Златиборски округ")
  ) {
    return "zlatibor_district";
  } else if (
    string.includes("Kolubara District") ||
    string.includes("Kolubara") ||
    string.includes("Колубарский") ||
    string.includes("Колубарски округ")
  ) {
    return "kolubara_district";
  } else if (
    string.includes("Moravica District") ||
    string.includes("Moravica") ||
    string.includes("Моравичский") ||
    string.includes("Моравички округ")
  ) {
    return "moravica_district";
  } else if (
    string.includes("Nišava District") ||
    string.includes("Nišava") ||
    string.includes("Нишавский") ||
    string.includes("Нишавски округ")
  ) {
    return "nisava_district";
  } else if (
    string.includes("Pirot District") ||
    string.includes("Pirot") ||
    string.includes("Пиротский") ||
    string.includes("Пиротски округ")
  ) {
    return "pirot_district";
  } else if (
    string.includes("Podunavlje District") ||
    string.includes("Podunavlje") ||
    string.includes("Подунайский") ||
    string.includes("Подунавски")
  ) {
    return "podunavlje_district";
  } else if (
    string.includes("Pčinja District") ||
    string.includes("Pčinja") ||
    string.includes("Пчиньский") ||
    string.includes("Пчињски")
  ) {
    return "pcinja_district";
  } else if (
    string.includes("Raška District") ||
    string.includes("Raška") ||
    string.includes("Рашский") ||
    string.includes("Рашки")
  ) {
    return "raska_district";
  } else if (
    string.includes("Rasina District") ||
    string.includes("Rasina") ||
    string.includes("Расинский") ||
    string.includes("Расински")
  ) {
    return "rasina_district";
  } else if (
    string.includes("Toplica District") ||
    string.includes("Toplica") ||
    string.includes("Топличский") ||
    string.includes("Топлички")
  ) {
    return "toplica_district";
  } else if (
    string.includes("Šumadija District") ||
    string.includes("Šumadija") ||
    string.includes("Шумадийский") ||
    string.includes("Шумадијски")
  ) {
    return "sumadija_district";
  } else if (
    string.includes("Jablanica District") ||
    string.includes("Jablanica") ||
    string.includes("Ябланичский") ||
    string.includes("Јабланички")
  ) {
    return "jablanica_district";
  } else if (
    string.toLowerCase().includes("zagreb city") ||
    string.toLowerCase().includes("град загреб") ||
    string.toLowerCase().includes("загреб") ||
    string.toLowerCase().includes("zagreb")
  ) {
    return "zagreb_city";
  } else if (
    string.toLowerCase().includes("zagrebačka županija") ||
    string.toLowerCase().includes("загребская")
  ) {
    return "zagreb_county";
  } else if (
    string.toLowerCase().includes("splitsko-dalmatinska županija") ||
    string.toLowerCase().includes("сплитско-далматинская")
  ) {
    return "split_dalmatia";
  } else if (
    string.toLowerCase().includes("istarska županija") ||
    string.toLowerCase().includes("истарская")
  ) {
    return "istria";
  } else if (
    string.toLowerCase().includes("primorsko-goranska županija") ||
    string.toLowerCase().includes("приморско-горанская")
  ) {
    return "primorje_gorski_kotar";
  } else if (
    string.toLowerCase().includes("ličko-senjska županija") ||
    string.toLowerCase().includes("лика-сень") ||
    string.toLowerCase().includes("lika-senj")
  ) {
    return "lika_senj";
  } else if (
    string.toLowerCase().includes("virovitičko-podravska županija") ||
    string.toLowerCase().includes("вировитицко-подравская")
  ) {
    return "virovitica_podravina";
  } else if (
    string.toLowerCase().includes("požeško-slavonska županija") ||
    string.toLowerCase().includes("пожешко-славонская") ||
    string.toLowerCase().includes("požega-slavonia")
  ) {
    return "pozega_slavonia";
  } else if (
    string.toLowerCase().includes("brodsko-posavska županija") ||
    string.toLowerCase().includes("бродско-посавская")
  ) {
    return "brod_posavina";
  } else if (
    string.toLowerCase().includes("zadarska županija") ||
    string.toLowerCase().includes("задар")
  ) {
    return "zadar";
  } else if (
    string.toLowerCase().includes("osječko-baranjska županija") ||
    string.toLowerCase().includes("осиечко-бараньская")
  ) {
    return "osijek_baranja";
  } else if (
    string.toLowerCase().includes("sisačko-moslavačka županija") ||
    string.toLowerCase().includes("сисачко-мославинская")
  ) {
    return "sisak_moslavina";
  } else if (
    string.toLowerCase().includes("koprivničko-križevačka županija") ||
    string.toLowerCase().includes("копривницко-крижевечка")
  ) {
    return "koprivnica_krizevci";
  } else if (
    string.toLowerCase().includes("bjelovarsko-bilogorska županija") ||
    string.toLowerCase().includes("бьеловарско-билогорская") ||
    string.toLowerCase().includes("bjelovar-bilogora")
  ) {
    return "bjelovar_bilogora";
  } else if (
    string.toLowerCase().includes("karlovačka županija") ||
    string.toLowerCase().includes("карловацкая") ||
    string.toLowerCase().includes("karlovac")
  ) {
    return "karlovac";
  } else if (
    string.toLowerCase().includes("varaždinska županija") ||
    string.toLowerCase().includes("вараждинская")
  ) {
    return "varazdin";
  } else if (
    string.toLowerCase().includes("krapinsko-zagorska županija") ||
    string.toLowerCase().includes("крапинско-загорская")
  ) {
    return "krapina_zagorje";
  } else if (
    string.toLowerCase().includes("međimurska županija") ||
    string.toLowerCase().includes("меджимурская")
  ) {
    return "medimurje";
  } else if (
    string.toLowerCase().includes("šibensko-kninska županija") ||
    string.toLowerCase().includes("шибенско-книнская")
  ) {
    return "sibenik_knin";
  } else if (
    string.toLowerCase().includes("vukovarsko-srijemska županija") ||
    string.toLowerCase().includes("вуковарско-сремская")
  ) {
    return "vukovar_srijem";
  } else if (
    string.toLowerCase().includes("dubrovačko-neretvanska županija") ||
    string.toLowerCase().includes("дубровачко-неретванская")
  ) {
...
const MapComponent = ({
      ...
      geocoder.geocode({ location: newCoordinates }, (results, status) => {
        if (status === "OK") {
          if (results[0]) {
            ...
            // }
            const { country, region, extRegion } =
              lservice.extractCountryAndRegion(results[0]);
  ...
  function getSerbianRegionKey(string) {
    ...
    const cleanedString = normalizedString
      .replace(/\bokrug\b|\bокруг\b/g, "")
      .trim();
  ...
  const extractCountryAndRegion = (geocodeResult) => {
    ...
    geocodeResult.address_components.forEach((component) => {
      if (
        country.toLowerCase().includes("bosnia") ||
        ...
        if (
          country.includes("Сербия") ||
          country.includes("Serbia") ||
          country.includes("Србиja")
        ) {
  ...
  return (
    <div>
      ...
      >
        <RegionSelector
          region={testRegion}
          setRegion={setTestRegion}
          country={testCountry}
          setCountry={setTestCountry}
  ...
  const handleSelect = async (value) => {
    ...
    if (selectedPlace) {
      ...
      if (
        ...
        try {
          const geocodeData = await fetchGeocodingData(
            newCoordinates.lat,
            newCoordinates.lng
          );
          if (geocodeData) {
            ...
            rules={[
              { required: true, message: "Please input the price!" },
              {
                validator: (_, value) => {
                  ...
                  if (!currency) {
                    return Promise.reject(
                      new Error("Please select a currency!")
                    );
                  }
          ...
          <Form.Item style={{ display: "flex", justifyContent: "center" }}>
            <Spin style={{ color: "#03989F" }} spinning={loading}>
              <Button
                ...
                onClick={onSubmit}
                size="large"
                style={{ backgroundColor: "#FFBF34", width: "150px" }}
              >
                {t("add")}
              </Button>
            </Spin>
