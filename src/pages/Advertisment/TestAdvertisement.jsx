...
import { fetchAdvertisments, resizeImageFromUrl } from "../../services/AdvertismentsHome/test";
...
const TestAdvertisment = () => {
    ...
    const applyFilters = async () => {
        ...
        const { advertisments: newAds, lastDoc } = await fetchAdvertisments(
            null, category, subcategory, country, region, condition, size, type, wheel, mileage, body, drive, year, transmission, memory, screen_size, brand, minPrice, maxPrice, currency
        );

        const resizedData = await Promise.all(newAds.map(async ad => {
            if (!ad.photoUrls || ad.photoUrls.length === 0) {
                return ad;
            }
            const resizedPhotoUrls = await Promise.all(ad.photoUrls.map(async url => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (fetching && hasMore) {
                ...
                const { advertisments: newAds, lastDoc } = await fetchAdvertisments(lastVisible, category, subcategory, country, region,
    ...
    return (
        <>
            ...
            <div className="container d-flex justify-end mt-3 mb-3">
                <OrangeButton onClick={showModalFilter} width='100px' heigth='50px' title={t('filter')} />
            </div>
